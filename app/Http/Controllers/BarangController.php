...
class BarangController extends Controller
{
    ...
    public function import_ajax(Request $request)
    {
        if ($request->ajax() || $request->wantsJson()) {
            $rules = [
                'file_barang' => ['required', 'mimes:xlsx', 'max:1024'] // validasi file harus xls atau xlsx, max 1MB
            ];
            ...
            if (count($data) > 1) { // jika data lebih dari 1 baris
                ...
                if (count($insert) > 0) {
                    BarangModel::insertOrIgnore($insert); // insert data ke database, jika data sudah ada, maka diabaikan
                }
